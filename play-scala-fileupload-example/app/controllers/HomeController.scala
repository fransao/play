...
class HomeController @Inject() (cc:MessagesControllerComponents)(implicit executionContext: ExecutionContext) extends MessagesAbstractController(cc) {
  ...
  def upload = Action(parse.multipartFormData(handleFilePartAsFile)) { implicit request =>
    val fileOption = request.body.file("name").map {
      case FilePart(key, filename, contentType, file) =>
        ...
        // TODO FS fail create file from temporary file C:\Users\francisco.salazar\AppData\Local\Temp
        //val newFile = new File("C:/" + file.getName);
  ...
  def downloadFile = Action {
    Ok.sendFile(
      content = new java.io.File("D:/proyectos/learnScala/Akka_With_Scala.pdf"),
